...
const AuctionItem = ({ isLoveSauceAlumni }: any) => {
  return (
    <Card
      h='fit-content'
      m='25px 0'
    ...
    >
      ...
      <CardFooter mt='25px' w='100%' p='0' justifyContent={'center'}>
        <Button
          ...
          p={['0', '30px 50px']}
          _hover={{
            background: isLoveSauceAlumni ? 'brand.600' : 'white',
            color: isLoveSauceAlumni ? 'white' : 'brand.900',
          }}
          background={isLoveSauceAlumni ? 'brand.600' : 'brand.900'}
        >
          <Text
            textTransform={'uppercase'}
            fontWeight={'medium'}
            fontSize={['md', 'lg', 'xl']}
          >
            {isLoveSauceAlumni ? 'lovesauce alumni' : 'add to program'}
          </Text>
        </Button>
      </CardFooter>
    </Card>
