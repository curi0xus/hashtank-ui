...
const AuctionItem = ({ img, isOffspring }: any) => {
  return (
    <Card
      h='fit-content'
      m='25px 0'
    ...
    >
      ...
      <CardFooter mt='25px' w='100%' p='0' justifyContent={'center'}>
        <Button
          ...
          p={['0', '30px 50px']}
          _hover={{
            background: isOffspring ? 'brand.600' : 'white',
            color: isOffspring ? 'white' : 'brand.900',
          }}
          background={isOffspring ? 'brand.600' : 'brand.900'}
        >
          <Text
            textTransform={'uppercase'}
            fontWeight={'medium'}
            fontSize={['md', 'lg', 'xl']}
          >
            {isOffspring ? 'offspring' : 'add to program'}
          </Text>
        </Button>
      </CardFooter>
    </Card>
