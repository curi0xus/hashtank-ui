...
const useRedeemSauce = (nftId: number) => {
  ...
  useEffect(() => {
    if (sauceNFTContractInstance && nftId) initRedeem();
  ...
  async function estimateRedeem() {
    ...
    if (sauceNFTContractInstance && nftId) {
      const gasLimit = await sauceNFTContractInstance.estimateGas.redeemSauce(
        BigInt(nftId),
        {
          customData: {
  ...
  async function redeem(nftId: number) {
    const transactionOpts = await estimateRedeem();
    if (sauceNFTContractInstance && transactionOpts) {
      await (
        await sauceNFTContractInstance.redeemSauce(
          BigInt(nftId),
          transactionOpts
        )
      ).wait();
